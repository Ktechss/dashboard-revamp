...
export default function Sidebar({ selectedOrganization, onSelectOrganization }) {
  ...
  // Mock data - one government and one private organization
  const organizations = [
    {
      id: 'gov-1',
      name: 'Dubai Police',
      ...
      phone: '+971501234567',
      // Government orgs have all validation services enabled by default
      validation_config: {
    ...
    },
    {
      ...
      phone: '+971509876543',
      // Private org with specific validation services enabled
      validation_config: {
        eid_validation: { enabled: true },
  ...
  // Platform navigation
  const platformNav = [
    { icon: Search, label: 'Government Search', page: 'GovernmentSearch' },
    { icon: BarChart3, label: 'Analytics', page: 'Analytics' },
    { icon: FileText, label: 'Dashboard Audit Logs', page: 'AuditLogs' },
    { icon: CheckSquare, label: 'Adjudicate Tasks', page: 'AdjudicateTasks' },
    { icon: Settings, label: 'Settings', page: 'Settings' }
  ];

  // Check if validation services are enabled for the selected organization
  const hasValidationService = selectedOrganization?.validation_config?.eid_validation?.enabled ||
    selectedOrganization?.validation_config?.passport_validation?.enabled;
  const hasBatchValidation = selectedOrganization?.validation_config?.batch_processing?.enabled;

  // Client-specific navigation (when organization is selected)
  const clientNav = [
    { icon: LayoutIcon, label: 'All Journeys', page: 'Journeys' },
    { icon: Fingerprint, label: 'Remote Verification', page: 'RemoteVerification' },
    { icon: GitBranch, label: 'One to Many', page: 'OneToMany' },
    { icon: Users, label: 'User Management', page: 'Users' },
    { icon: DollarSign, label: 'Balance & Metering', page: 'Balance' },
    { icon: FileText, label: 'API Info', page: 'ApiInfo' },
    { icon: DollarSign, label: 'API Pricing & Credits', page: 'Pricing' },
    { icon: RefreshCw, label: 'Population Migration', page: 'Migration' },
    { icon: Activity, label: 'Proactive Monitoring', page: 'Monitoring' },
    // Conditionally show Validation Transactions if EID or Passport validation is enabled
    ...(hasValidationService ? [{ icon: FileCheck, label: 'Validation Transactions', page: 'Validation' }] : []),
    // Conditionally show Batch Validation if batch processing is enabled
    ...(hasBatchValidation ? [{ icon: Layers, label: 'Batch Validation', page: 'BatchValidation' }] : []),
    { icon: Shield, label: 'Sandbox Residents', page: 'Sandbox' },
    { icon: Settings, label: 'Settings', page: 'Settings' },
  ...
  // Show client-specific navigation when an organization is selected
      ...
      <div className="w-64 h-screen bg-slate-900 text-white flex flex-col">
        ...
        <div className="p-4 bg-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold truncate">{selectedOrganization.name}</p>
              <p className="text-xs text-slate-400 capitalize">{selectedOrganization.type}</p>
            </div>
          </div>
        </div>
  ...
  // Default platform view (no organization selected)
  return (
    <div className="w-64 h-screen bg-slate-900 text-white flex flex-col">
      {/* Header */}
      <div className="p-4 border-b border-slate-800">
      ...
      <ScrollArea className="flex-1">
        ...
        <div className="p-4">
          ...
          <div className="space-y-1">
            ...
            <button className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-slate-300 hover:bg-slate-800 hover:text-white transition-all">
              <Search className="h-4 w-4" />
              <span className="text-sm font-medium">Quick Search</span>
            </button>
            <button
              onClick={() => setShowOnboardSheet(true)}
              className="w-full flex items-center gap-3 px-3 py-2 rounded-lg bg-sky-600 hover:bg-sky-700 text-white transition-all"
            >
              <Plus className="h-4 w-4" />
              <span className="text-sm font-medium">Onboard New Client</span>
            </button>
          </div>
        </div>
